...
do_install() {
  ...
  for lib in "${_gcc_libs[@]}"; do
    cp -av "$(pkg_path_for bdangit/gcc8)/lib/${lib}".* "$pkg_prefix/lib"/
  done
  rm -fv "$pkg_prefix/lib"/*.spec "$pkg_prefix/lib"/*.py

  mkdir -pv "$pkg_prefix/share/licenses"
  cp -av "$(pkg_path_for bdangit/gcc8)"/share/licenses/RUNTIME.LIBRARY.EXCEPTION \
