...
do_install() {
  CUDA_RUN_PATH="${pkg_prefix}/lib64:${pkg_prefix}/nvvm/lib64:$(pkg_path_for bdangit/gcc8-libs)/lib:$(pkg_path_for glibc)/lib"
  ...
  for lib in "${_cuda_libs[@]}"; do
    cp -av "$(pkg_path_for bdangit/cuda)/lib64/${lib}".* "${pkg_prefix}/lib64/"
  ...
  for lib in "${_cuda_stubs_libs[@]}"; do
    cp -av "$(pkg_path_for bdangit/cuda)/lib64/stubs/${lib}".* "${pkg_prefix}/lib64/stubs/"
    patchelf --set-rpath "${CUDA_RUN_PATH}" "${pkg_prefix}/lib64/stubs/${lib}.so"
  done

  cp -av "$(pkg_path_for bdangit/cuda)/nvvm/lib64/libnvvm".* "${pkg_prefix}/nvvm/lib64/"
  patchelf --set-rpath "${CUDA_RUN_PATH}" "${pkg_prefix}/nvvm/lib64/libnvvm.so"

  # Install EULA
  mkdir -p "${pkg_prefix}/share/licenses"
  cp -av "$(pkg_path_for bdangit/cuda)/share/licenses/EULA.pdf" "${pkg_prefix}/share/licenses/EULA.pdf"
}
